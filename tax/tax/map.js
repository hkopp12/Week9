...
//Create a feature layer from your URL
  var martialStatusUrl = "https://services.arcgis.com/P3ePLMYs2RVChkJx/ArcGIS/rest/services/ACS_10_14_Marital_Status_Boundaries/FeatureServer/0"
  const martialStatusLayer = new FeatureLayer({
    url: martialStatusUrl,
...
var loadAdditionalData =function(){
  ...
  let additionalQuery = martialStatusLayer.createQuery();
  additionalQuery.returnGeometry = false;
  additionalQuery.outFields = "*"
  ////execute query on your new layer
  martialStatusLayer.queryFeatures(additionalQuery).then(function(r){
console.log(r)
r.features.forEach(function(feature){
  ...
  if(feature.attributes['STUSPS'] != "DC"){
      ...
      stateObj[stateName]['martialStatus'] = Marital
  }
...
///////////Add new row here too //////////                            
                            maritalStatus: Math.trunc(maritalStatus).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","), 
